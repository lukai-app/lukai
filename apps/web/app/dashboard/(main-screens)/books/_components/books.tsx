...
import {
  groupByDate,
  sortGroupedByDate,
} from '@/app/dashboard/(main-screens)/books/utils';
...
interface BooksProps {
  ...
  month: number;
  year: number;
  selectedTab: 'journal' | 'ledger' | 'profit-loss' | 'balance';
}
...
export const Books: React.FC<BooksProps> = ({
  ...
  const [data, setData] = useState<DecryptedAccountingData | null>(null);
...
function JournalBook({
  ...
  currency: string;
  locale: string;
  setData: Dispatch<SetStateAction<DecryptedAccountingData | null>>;
}) {
  ...
  interface GroupedJournalEntries {
    [key: string]: JournalEntry[]; // key is the date
  }

  const groupJournalEntriesByDate = (journalEntries: JournalEntry[]) => {
    const groups: GroupedJournalEntries = {};
  ...
  const groupedJournalEntries = useMemo(
    () => groupJournalEntriesByDate(data.journalEntries),
    [data.journalEntries]
  );

  const sortGroupedJournalEntries = (
  ...
  return (
    <div className="flex-1 relative flex flex-col pb-8">
        ...
        <Table>
          <TableHeader className="sticky top-0 bg-[#05060A] border-0 border-gray-800">
            <TableRow className="!border-0 hover:bg-transparent">
              <TableHead className="w-[15%] h-10 border-0 bg-[#05060A]">
                fecha
              </TableHead>
              <TableHead className="w-[35%] h-10 border-0 bg-[#05060A]">
                movimiento
              </TableHead>
              <TableHead className="w-[25%] h-10 border-0 bg-[#05060A]">
                cuenta
              </TableHead>{' '}
              <TableHead className="w-[12.5%] h-10 text-right border-0 bg-[#05060A]">
                debe
              </TableHead>
              <TableHead className="w-[12.5%] h-10 text-right border-0 bg-[#05060A]">
                haber
          ...
          <TableBody className="overflow-y-auto">
            {sortGroupedJournalEntries(groupedJournalEntries)
              .reverse()
              .map(([label, entries], index) => (
                <React.Fragment key={label}>
                  <TableRow className="border-0">
                    <TableCell
                      className="max-w-0 py-[6px] px-2 rounded-t-[10px] bg-[#1a1a1a] hover:bg-[#1a1a1a]"
                      colSpan={5}
                    >
      ...
      <div className="flex z-10 bottom-4 justify-between items-center py-2 px-3 rounded-lg bg-black mt-4 border border-[#1a1a1a]">
        <h2 className="text-sm ml-auto">total</h2>
        <div className="grid grid-cols-2 gap-2 w-48 text-right">
...
const JournalEntryItem = ({
  ...
  const handleDelete = () => {
    toast.promise(
      ...
      }),
      {
        ...
        success: (response) => {
          return {
            message: `Transacción "${entry.description}" eliminada`,
            action: {
              label: 'Deshacer',
              onClick: () => {
  ...
  return (
    <TableRow
      key={entry.id}
      className="border-gray-800 hover:bg-muted -mx-3 px-3 group"
    >
      ...
      <TableCell className="max-w-0 p-0 h-[58px]  ">
        <div className="w-full h-full pt-[6px]">
          <p className="text-sm truncate">{entry.description}</p>
        </div>
      </TableCell>
      <TableCell className="p-0 h-[58px]">
        <div className="w-full h-full pt-[6px] p-1 flex flex-col justify-between">
          <p className="text-sm truncate">
            {entry.type === 'transfer'
              ? entry.accountFrom?.name
              : entry.type === 'income'
              ? `Ingresos (${entry.category.name})`
              : `Gastos (${entry.category.name})`}
          </p>
          <p className="text-sm truncate">
      ...
      <TableCell className="p-0 h-[58px]">
        <div className="w-full h-full pt-[6px] p-1 flex flex-col justify-between">
          ...
          <p
            className={cn(
              'text-right text-sm whitespace-nowrap p-1',
...
function LedgerAccountCard({
  ...
  const groupedEntries = useMemo(() => {
    ...
    const startingBalanceEntry: JournalEntry = {
      id: 'starting-balance',
      type: 'balance',
      description: 'saldo inicial',
      amount: 0,
      created_at: firstDayOfMonth.toISOString(),
      accountFrom: null,
      accountTo: null,
      category: { id: 'balance', name: 'balance' },
    };

    const allEntries = [startingBalanceEntry, ...entries].sort((a, b) => {
      return (
  ...
  }, [entries, account.startingBalance, account.id, locale, month, year]);

  return (
    <div className="flex-1 relative flex flex-col">
      <Table>
        <TableHeader className="bg-[#05060A] border-0 border-gray-800">
          <TableRow className="!border-0 hover:bg-transparent relative">
            <TableHead className="h-10 border-0 bg-[#05060A]">
              descripción
            </TableHead>
            <TableHead className="w-32 h-10 border-0 bg-[#05060A] text-right">
              debe
            </TableHead>
            <TableHead className="w-32 h-10 border-0 bg-[#05060A] text-right">
              haber
        ...
        <TableBody>
          ...
          <TableRow className="bg-transparent hover:bg-transparent">
            <TableCell className="!p-0 h-[38px]">
              <div className="py-2 px-2 text-sm text-right border-l border-t border-b rounded-l-lg border-[#1a1a1a]">
                total
              </div>
            </TableCell>
            <TableCell className="!p-0 h-[38px]">
              <div className="text-sm py-2 text-right text-lettuce border-t border-b border-[#1a1a1a]">
                {formatCurrency(totalDebit)}
              </div>
            </TableCell>
            <TableCell className="!p-0 h-[38px]">
              <div className="text-sm py-2 text-right text-yc border-t border-b border-[#1a1a1a]">
                {formatCurrency(totalCredit)}
              </div>
            </TableCell>
            <TableCell className="!p-0 h-[38px]">
              <div className="text-sm py-2 pr-2 whitespace-nowrap border-t border-b border-[#1a1a1a]-2 text-right text-white border-r rounded-r-lg border-[#1a1a1a]">
                {formatCurrency(
                  account.startingBalance + totalDebit - totalCredit
                )}
              </div>
            </TableCell>
          </TableRow>

...
import { importKey, decryptNumber, decrypt } from '@/lib/utils/encryption';
import { differenceInMonths, startOfYear } from 'date-fns';

import { useSession } from '@/app/_components/session-provider';
import { IHomeGetResponse } from '@/app/dashboard/_types/analytics';
...
const getHome = async (params: {
  ...
  const response = (await fetch(
    `${env.NEXT_PUBLIC_API_URL}/v1/home?year=${year}${
      ...
      try {
        ...
        const decryptedMonthData = {
          ...
          expensesByCategory: await Promise.all(
            rawData.data.monthData.expensesByCategory.map(async (category) => ({
              ...
              budget: category.budget
                ? (await decryptNumber(category.budget, cryptoKey)) || 0
                : null,
            }))
          ),
          ...
          expenses: await Promise.all(
            rawData.data.monthData.expenses.map(async (expense) => ({
              ...expense,
              amount: (await decryptNumber(expense.amount, cryptoKey)) || 0,
              description:
                (await decrypt(expense.description, cryptoKey)) || '',
            }))
          ),
          incomes: await Promise.all(
            rawData.data.monthData.incomes.map(async (income) => ({
              ...income,
              amount: (await decryptNumber(income.amount, cryptoKey)) || 0,
              description: (await decrypt(income.description, cryptoKey)) || '',
            }))
          ),
        };

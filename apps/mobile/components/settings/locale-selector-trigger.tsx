...
const updateLocaleFunction = async ({
  locale,
  token,
}: {
  locale: string;
  ...
  const response = await fetch(
    `${env.EXPO_PUBLIC_API_URL}/v1/users/update-locale`,
    {
      method: 'PATCH',
      headers: {
  ...
  if (!response.ok) {
    throw new Error(
      data.message || 'Error al actualizar la configuración regional'
    );
  }
...
export const LocaleSelectorTrigger = () => {
  ...
  return (
    <Pressable
      ...
      disabled={isPending}
      onPress={() => {
        SheetManager.show('locale-selector', {
          payload: {
            onSelect: (locale: string) => {
              SheetManager.hide('locale-selector');
              updateLocale({
                locale,
                token: session?.token ?? '',
              });
    ...
    >
      <View className="flex flex-row items-center">
        <View className="flex items-center justify-center w-10 h-10 rounded-full bg-gray-800 mr-4">
          <MapPin className="h-5 w-5" color="red" />
        </View>
        <View>
          <Text
